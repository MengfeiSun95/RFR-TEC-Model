...
class RFRModule(nn.Module):
    def __init__(self, layer_size=6, in_channel = 64):
        super(RFRModule, self).__init__()    ###调用父类
    ...
    def forward(self, input, mask):
        ...
        for i in range(self.layer_size - 1, 0, -1):
            ...
            ######
            # print('h:',h.shape)
            # print('h_dict[enc_h_key]%s:'%enc_h_key,h_dict[enc_h_key].shape)
            h = torch.cat([h, h_dict[enc_h_key]], dim=1)
            # print('over  h :',h.shape)
...
class RFRNet(nn.Module):
    def __init__(self):
        ...
        # in_channel \out channel\kernel_size\strid\padding
        self.Pconv1 = PartialConv2d(3, 64, 7, 2, 3, multi_channel = True, bias = False)
        # self.Pconv1 = PartialConv2d(3, 64, 7, 2, (41,79), multi_channel = True, bias = False)
