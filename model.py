...
class RFRNetModel():
    ...
    def train(self, train_loader, save_path, finetune = True, iters=500000):

    #    writer = SummaryWriter(log_dir="log_info")
        m=self.G.train(finetune = finetune)
        ...
        while self.iter<iters:   
            for items in train_loader:
                ...
                if self.iter % 500 == 0:
                    ...
                    print("Iteration:%d, l1_loss:%.4f, time_taken:%.2f" %(self.iter, self.l1_loss_val/500, int_time))
                    s_time = time.time()
                    self.l1_loss_val = 0.0
    ...
    # def test(self, test_loader, result_save_path,save_nama,datatime):
    def test(self, test_loader, result_save_path,save_name):
        self.G.eval()
        ...
        for items in test_loader:
            ...
            for k in range(comp_B.size(0)):
                ...
                file_path = '{:s}/results/dens_img_{:s}.png'.format(result_save_path,save_name)
                # file_path = '{:s}/results/dens_img_{:d}{:s}_{:s}.png'.format(result_save_path, count,save_nama,datatime)
                save_image(grid, file_path)



                grid = make_grid(masked_images[k:k+1] +1 - masks[k:k + 1] )
                file_path = '{:s}/results/masked_img_{:s}.png'.format(result_save_path,save_name)
                # file_path = '{:s}/results/masked_img_{:d}{:s}_{:s}.png'.format(result_save_path, count,save_nama,datatime)
                save_image(grid, file_path)

                # mask_1=masks[k:k + 1]
                # grid = make_grid(mask_1)
                # file_path = '{:s}/results/mask_{:d}.png'.format(result_save_path, count)
                # save_image(grid, file_path)
